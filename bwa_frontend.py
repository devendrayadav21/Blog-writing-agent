...
# Caps how many "worker" section-writer calls run in parallel during fan-out.
# Groq's free/on-demand tier has a tight tokens-per-minute (TPM) budget, and
# firing all section calls at once easily blows through it. Lower this further
# (e.g. to 1) if you still hit 429s; raise it if you're on a paid Groq tier.
GRAPH_CONFIG = {"max_concurrency": 2}
...
def try_stream(graph_app, inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    ...
    """
    try:
        for step in graph_app.stream(inputs, stream_mode="updates", config=GRAPH_CONFIG):
            yield ("updates", step)
        out = graph_app.invoke(inputs, config=GRAPH_CONFIG)
        yield ("final", out)
        return
    except Exception:
        pass
    ...
    try:
        for step in graph_app.stream(inputs, stream_mode="values", config=GRAPH_CONFIG):
            yield ("values", step)
        out = graph_app.invoke(inputs, config=GRAPH_CONFIG)
        yield ("final", out)
        return
